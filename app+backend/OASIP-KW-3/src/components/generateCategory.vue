...
<template>
  <div>
    <div class="bg-white shadow-xl rounded-b-lg ml-24 mr-24 p-12">
      ...
      <router-link :to="{ name: 'ListAllEvent', query: { categoryId: 0 } }">
        <div v-if="type == `ShowList`" class="text-center">Check all Appointment </div>
      </router-link>
      <div class="grid grid-cols-3 gap-6 place-items-center my-12">
        ...
        <div v-for="category in categorys" :key="category.eventCategoryID">
          ...
          <div>
            <div @click="addList(category.eventCategoryID, category.eventCategoryName, category.eventDuration)"
